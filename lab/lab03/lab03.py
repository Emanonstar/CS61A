...
def pingpong(n):
    ...
    def pingpong_helper(value, index, direction):
        ...
        if index % 8 == 0 or num_eights(index) != 0:
            return pingpong_helper(value - direction + (not direction), index + 1, (not direction))
        return pingpong_helper(value + direction - (not direction), index + 1, direction)
    
    return pingpong_helper(1, 1, True)
    
    
    # Tree Recursion Method 
    # Problem: Not apply for n grows big 
    # if n <= 2:
    #     return n
