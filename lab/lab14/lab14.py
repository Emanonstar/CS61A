def prune_min(t):
    ...
    if t.is_leaf():
        return
    if len(t.branches) != 2: 
        return 'Invalid Tree: Tree must have zero or two branches'
    b1, b2 = t.branches[0], t.branches[1]
    if b1.label < b2.label:
        b = b1
    elif b1.label > b2.label:
        b = b2
    else:
        return "Invalid Tree: label value of branches can't equal"
    t.branches = [b]
    prune_min(b)
...
def num_splits(s, d):
    ...
    #         return 0
    #     if len(s) == 1:
    #         if (s[0] > l and s[0] <= d) or (s[0] <= -l and s[0] >= -d):
    #             return 1
    #         else:
    #             return 0
    #     return helper(s[1:], d+s[0], l+s[0]) + helper(s[1:], d-s[0], l-s[0])
    # return helper(s, d, 0)
    def helper(s, s1, s2):
        if s == []:
            if abs(s1-s2) <= d:
